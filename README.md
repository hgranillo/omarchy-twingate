...
on the row under the cursor. Hovering shows the full address and auth time left.
...
hides while you filter. A name that no longer resolves is dropped.

### Long Resource Lists

Resources needing authentication pin to the top, then recents, search box above
...
They are pinned in `NEEDS AUTHENTICATION` and counted in the header.
...
**Per resource.** Some resources need their own authorization. They are pinned in
`NEEDS AUTHENTICATION`. Activating one runs `twingate-notifier auth <name>` and
...
account session policy flow, so the nearest `auth_expires_at` among them is the
...
additionally block reading stdin.
...
that prompt, scoped to `twingate.service` and your user. Nothing installs it. The
cost: any process running as you can then stop your tunnel silently.
...
presented terminal, so you see the command before it runs; turning on sign-in
prompts is a user unit and runs directly.
...
| Key | Default | Meaning |
|---|---|---|
| `statusIntervalSec` | `10` | Status poll interval. The daemon has been seen to stop answering under frequent polling, so keep it conservative. |
...
Placement takes `--section`, `--index`, and `--before` / `--after`. The latter two
fail unless the bar already carries the widget you name, so `--section` is the
portable form.

Editing a bar widget needs a restart. Saving logs a reload, but the running bar
keeps the component it loaded first, and the first `IpcHandler` registration owns
its target for the process lifetime.
...
payload with fixed timestamps, covering shapes a healthy network never shows: an
expired authorization, a hidden resource, a wildcard address.

`qmllint` exits non-zero with no output on any file using inline `component`
declarations, including Omarchy's own panels. Use it on `Service.qml` and
`TwingateIcon.qml` only.

**Validate** runs the suite on every push and pull request, and checks the
install line above still points here. **Release** fires on a `v*` tag: it tests,
refuses a tag that disagrees with `manifest.json`, skips an existing release, and
writes notes from the commits since the previous tag. A release is a version
